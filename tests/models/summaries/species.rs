...
use polaris::models::summaries::{SpecieSummary, SpecieSummaryExtract};
use rstest::rstest;
...
macro_rules! configure_insta {
    ($(expr:expr),*) => {
        ...
        settings.set_snapshot_suffix("species");
...
async fn can_extract_summary_data(
    ...
    let result = SpecieSummaryExtract::get_from_animals(&ctx.db, org_pid, specie).await;
...
async fn can_generate_summary(
    ...
    let result = SpecieSummary::generate(&ctx.db, org_pid, specie).await;
