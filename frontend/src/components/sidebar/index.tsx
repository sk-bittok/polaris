    ...
    return (
        <div className={`fixed flex flex-col h-full ${isSidebarCollapsed ? "w-0 md:w-16" : "w-72 md:w-64"} shadow-md rounded-tr-2xl rounded-br-2xl overflow-hidden bg-white dark:bg-black z-40 transition-all duration-300`}>
            ...
            <div className={`flex gap-3 justify-between md:justify-normal items-center pt-8 ${isSidebarCollapsed ? "px-5" : "px-8"} `}>
                ...
                <h1 className={`font-extrabold text-2xl ${isSidebarCollapsed ? "hidden" : "block"}`}>Polaris</h1>

                <button onClick={() => toggleSidebar()} type="button" className="md:hidden p-3 bg-gray-200 dark:bg-gray-800 rounded-full hover:bg-blue-200 dark:hover:bg-blue-800">
            ...
            <div className="flex-grow mt-8">
                ...
                <SidebarLink href="/livestock" isSidebarCollapsed={isSidebarCollapsed} icon={Rabbit} label="Livestock" />
            </div>

            {/* FOOTER SECTION */}
            <div className={`${isSidebarCollapsed ? "hidden" : "block"}  mb-10`}>
                <p className="text-center text-xs">&copy; {new Date().getFullYear()} Polaris</p>
            </div>
        </div>
    )
}
