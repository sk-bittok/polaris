...
export function RegisterForm({
  ...
  const onCompanySubmit = (data: CompanyFormSchema) => {
    setFormData((prev) => ({
      ...prev,
      organisation: data,
    }));
  ...
  const onUserSubmit = async (data: UserFormSchema) => {
    setFormData((prev) => ({
      ...prev,
      user: data,
    }));
    ...
    return (
      <div className="flex items-center justify-center mb-8">
        <div className="flex items-center">
          <div
            className={`flex items-center justify-center w-10 h-10 rounded-full ${currentStep >= 1 ? "bg-blue-600" : "bg-gray-300"} text-white`}
          >
            <Building size={24} />
          </div>
          <div
            className={`h-1 w-12 ${currentStep > 1 ? "bg-blue-600" : "bg-gray-300"}`}
          />
          <div
            className={`flex items-center justify-center w-10 h-10 rounded-full ${currentStep >= 2 ? "bg-blue-600" : "bg-gray-300"} text-white`}
          >
            <User size={24} />
          </div>
          <div
          ...
          />
          <div
            className={`flex items-center justify-center w-10 h-10 rounded-full ${currentStep >= 3 ? "bg-blue-600" : "bg-gray-300"} text-white`}
          >
      ...
      <Form {...companyForm}>
        ...
        >
          ...
          <CustomFormField
            ...
            name="companyEmail"
            label="Company email"
            placeholder="company@mail.org"
          />
          <CustomFormField
            control={companyForm.control}
            name="companyAddress"
            label="Company address"
            placeholder="25 Privet Drive"
          />
          <CustomFormField
            control={companyForm.control}
            name="companyPhone"
  ...
  const SuccessStep = () => {
    return (
      <div className="text-center py-8">
        ...
        <p>
          Thank you for registering {formData?.user.firstName}. &nbsp;Your
          account for {formData?.organisation.companyName} has been created
          successfully
        </p>
        <Button
