...
export const updateLivestockSchema = z.object({
	...
	gender: z
		.enum([Gender.Male, Gender.Female, Gender.Unkown])
		.optional()
		.nullable(),
	status: z
		.enum([Status.Transferred, Status.Active, Status.Sold, Status.Deceased])
		.optional()
		.nullable(),
...
export const linkOffspringSchema = z.object({
	offspringTagId: z
		.string()
		.min(2, { message: "Tag ID requires 2 letters" })
		.max(20, { message: "Tag ID must be under 20 letters" }),
	offspringName: z.string().optional().nullable(),
	parentTagId: z
		.string()
		.min(2, { message: "Tag ID requires 2 letters" })
		.max(20, { message: "Tag ID must be under 20 letters" }),
	parentGender: z.enum(["male", "female"], { message: "male or female only" }),
});

export type LinkOffspring = z.infer<typeof linkOffspringSchema>;
